...
class Field:
    ...
    def __init__(self, map_name):
        ...
        we have 26 x 26 battle field
        '''
        borders = 2
        self.size = 26 + borders
        self.map_name = map_name
        self.field = np.zeros((self.size, self.size), np.int32).reshape(
                              self.size, self.size)
        for i in range(self.size):
            self.field[0][i] = self.field[i][0] = (
                self.field[i][self.size - 1]) = (
                    self.field[self.size - 1][i]) = 125
        self.read_map()
    ...
    def read_map(self):
        ...
        for y in range(0, self.size - 2):
            for x in range(0, self.size - 2):
                ...
                self.field[y + 1][x + 1] = ord(map_lines[y][x])
            print()
